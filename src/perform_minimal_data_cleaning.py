"""Create the interim CKD dataset from the raw CSV input."""
...
def perform_minimal_data_cleaning():
    ...
    df = pd.read_csv(RAW_DATA_PATH, header=0, skiprows=[1, 2])

    # Remove the row from the 'grf' column containing the value 'p',
    # avoiding assumptions about what 'p' represents in the dataset,
