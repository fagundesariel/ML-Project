...
preprocessing workflow was applied inside each fold to avoid data leakage. In
this comparison, no dimensionality reduction and no balancing strategy were
applied, matching the simpler configuration selected after the preprocessing
experiments.
...
obtained precision of 1.0000, recall of 0.9861, and F1-score of 0.9930. SVM
and Naive Bayes also performed well, but Random Forest showed the best balance
between predictive performance and stability across folds.
Table~\ref{tab:model_comparison} summarizes the main metrics, while
Figure~\ref{fig:model_comparison_results} presents the F1-score distribution
and ROC curves.
...
variance achieved the best average F1-score across classifiers, while PCA
retaining 95\% of the variance achieved the best Random Forest F1-score.

For the final Random Forest configuration, however, the difference between PCA
95\% and no dimensionality reduction was very small: 0.9935 versus 0.9926 in
mean F1-score. This marginal gain was not considered sufficient to justify the
additional computational cost or the loss of direct feature interpretability
introduced by PCA. As a result, PCA was not adopted in the final selected
model.
...
dimensionality reduction and PCA retaining 95\% of the variance. The best
numerical result was obtained with PCA 95\% combined with either the baseline
configuration or the focused grid search, both reaching mean F1-score of
0.9935. The no-dimensionality-reduction baseline and grid-search
configurations reached mean F1-score of 0.9926, with mean accuracy of 0.9949,
mean precision of 1.0000, and mean recall of 0.9857.
...
additional computational cost of PCA and hyperparameter search. It also keeps
...
model-dependent predictive evidence rather than causal clinical effects. The
